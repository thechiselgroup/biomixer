...
public class CallbackException extends Exception {
    ...
    public String getMessage() {
        return (isCausedByTimeoutException() ? "server timeout - " : "")
                + super.getMessage();
    }

    public boolean isCausedByTimeoutException() {
        if (getCause() instanceof TimeoutException) {
            return true;
        }

        if (getCause() instanceof CallbackException) {
            return ((CallbackException) getCause())

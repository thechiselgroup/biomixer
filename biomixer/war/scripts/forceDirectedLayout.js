function forceDirectedLayout(div, json){
		...
		function drawLayout(json) {
			...
			var force = self.force = d3.layout.force().nodes(json.nodes)
		...
		function highlightLink() {
			return function(d, i) {
				...
				d3.selectAll("text").style("opacity", .2).filter(
						function(g, i) {
				...
				d3.selectAll("circle").style("fill-opacity", .1).style(
						"stroke-opacity", .2).filter(
						function(g, i) {
							return g.x == d.source.x || g.y == d.source.y
									|| g.x == d.target.x || g.y == d.target.y
						}).style("fill-opacity", 1).style("stroke-opacity", 1);
				d3.select(this).style("stroke-opacity", 1).style("stroke",
						"#3d3d3d");
		...
		function changeColour(circleFill, lineFill, circlesFill, opacity) {
			return function(d, i) {
				...
				var adjacentLinks = d3.selectAll("line").filter(
						function(d, i) {
		...
		function changeColourBack(circleFill, lineFill) {
			return function(d, i) {
				d3.selectAll("circle").style("fill", circleFill).style(
						"fill-opacity", .75).style("stroke-opacity", 1);
				d3.selectAll("line").style("stroke", lineFill).style(
						"stroke-opacity", .75);

...
define(["require", "exports", "../GraphView", "../Menu", "./ConceptGraph", "../JQueryExtension", "LayoutProvider", "GraphView", "Menu", "Concepts/ConceptPathsToRoot", "Concepts/ConceptGraph"], function(require, exports, GraphView, Menu, ConceptGraph) {
    var ConceptLayouts = (function () {
        ...
        ConceptLayouts.prototype.addMenuComponents = function (menuSelector) {
            ...
            var forceButton = $("<input>").attr("class", "layoutButton").attr("id", "forceLayoutButton").attr("type", "button").attr("value", "Force-Directed Layout");

            var circleButton = $("<input>").attr("class", "layoutButton").attr("id", "circleLayoutButton").attr("type", "button").attr("value", "Circle Layout");

            var centerButton = $("<input>").attr("class", "layoutButton").attr("id", "centerLayoutButton").attr("type", "button").attr("value", "Center Layout");

            var horizTreeButton = $("<input>").attr("class", "layoutButton").attr("id", "horizontalTreeLayoutButton").attr("type", "button").attr("value", "Horizontal Tree Layout");

            var vertTreeButton = $("<input>").attr("class", "layoutButton").attr("id", "verticalTreeLayoutButton").attr("type", "button").attr("value", "Vertical Tree Layout");

            var radialButton = $("<input>").attr("class", "layoutButton").attr("id", "radialLayoutButton").attr("type", "button").attr("value", "Radial Layout");

            var importButton = $("<input>").attr("class", "layoutButton").attr("id", "importedLayoutButton").attr("type", "button").attr("value", "Imported Layout");
        ...
        ConceptLayouts.prototype.getLayoutPositionSnapshot = function () {
            ...
            $.each(graphNodes, function (index, node) {
                positions[String(node.rawConceptUri)] = { x: node.x, y: node.y };
            });
            return positions;
        };
        ...
        */
        ConceptLayouts.prototype.transitionNodes = function (refresh, transitionOnlyFixedNodes) {
            ...
            d3.selectAll(GraphView.BaseGraphView.linkSvgClass).transition().duration(duration).ease("linear").attr("points", outerThis.graphView.updateArcLineFunc);

            d3.selectAll(GraphView.BaseGraphView.linkMarkerSvgClass).transition().duration(duration).ease("linear").attr("points", outerThis.graphView.updateArcMarkerFunc);
        ...
        ConceptLayouts.prototype.runForceLayoutLambda = function () {
            ...
            return function (refreshLayout) {
                ...
                outerThis.forceLayout.friction(0.3).gravity(0.05).charge(-100); // -100
